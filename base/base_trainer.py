...
class Multi_BaseTrainer_dist:
    ...
    def _save_checkpoint(self, epoch, save_best=False, save_runtime=False, iteration=None):
        """
        Saving checkpoints
        ...
        if save_runtime:
            ...
            filename = str(self.checkpoint_dir / 'checkpoint-runtime-epoch{}.pth'.format(epoch))
        else:
            filename = str(self.checkpoint_dir / 'checkpoint-epoch{}.pth'.format(epoch))
        torch.save(state, filename)
        self.logger.info("Saving checkpoint: {} ...".format(filename))
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        self.start_epoch = checkpoint['epoch'] + 1
        # self.mnt_best = checkpoint['monitor_best']
        self.mnt_best = checkpoint.get('monitor_best', 0.0)
        ...
        try:
            ...
            self.logger.warning(f"Warning: config not a key of checkpoint {resume_path}, pass.")
        ...
        try:
            ...
            self.logger.warning(f"Warning: config-optimizer not a key of checkpoint {resume_path}, pass.")
