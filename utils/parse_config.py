...
class ConfigParser:
    def __init__(self, args, options='', timestamp=True, test=False, eval_mode=None):
        ...
        if args.resume is None:
            ...
            if eval_mode == "epic":
                resume_cfg_fname = Path(f'{work_dir}/configs/eval/epic.json')
            if eval_mode == "nlq":
                resume_cfg_fname = Path(f'{work_dir}/configs/eval/nlq.json')
            if eval_mode == "mq":
                resume_cfg_fname = Path(f'{work_dir}/configs/eval/mq.json')
        ...
        timestamp = datetime.now().strftime(r'%m%d_%H_%M') if timestamp else ''
